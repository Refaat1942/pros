    ...
    private function currentStepIndex(?string $stageKey, bool $isMilitary, bool $noCase): int
    {
        ...
        if ($isMilitary) {
            return match ($stageKey) {
                ...
                CaseRecord::STAGE_ADMIN_APPROVAL  => 2,
        ...
        return match ($stageKey) {
            ...
            CaseRecord::STAGE_WAITING_RETURN  => 3,
    ...
    private function publicStageLabel(?string $stageKey, bool $noCase, bool $isMilitary): string
    {
        ...
        if (! $isMilitary) {
            return match ($stageKey) {
                ...
                CaseRecord::STAGE_COST_CALC      => 'جاري تجهيز التسعير',
                CaseRecord::STAGE_ADMIN_APPROVAL => 'بانتظار اعتماد الإدارة على التسعير',
                CaseRecord::STAGE_QUOTE          => 'عرض السعر — بانتظار موافقة الجهة الضامنة',
                CaseRecord::STAGE_WAITING_RETURN => 'بانتظار موافقة الجهة الضامنة',
        ...
        return match ($stageKey) {
            ...
            CaseRecord::STAGE_EXAM           => 'في مرحلة الكشف الطبي',
            CaseRecord::STAGE_TECHNICAL      => 'التوصيف الفني',
            CaseRecord::STAGE_COST_CALC      => 'حساب التكلفة — انتقال للورشة',
            CaseRecord::STAGE_ADMIN_APPROVAL => 'التوصيف الفني',
            CaseRecord::STAGE_MANUFACTURING  => 'جاري التصنيع بالورشة',
            CaseRecord::STAGE_READY_DELIVERY => 'جاهز للتسليم',
            CaseRecord::STAGE_DELIVERED      => 'تم التسليم',
            default                          => CaseStage::labelFor($stageKey),
