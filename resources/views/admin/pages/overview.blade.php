<div class="section-view overview-page" id="section-overview" data-server-rendered="1">
    ...
    @if (! empty($cycle_cards) || isset($cycle_total_active))
        <section class="overview-section overview-section--cycle" aria-labelledby="overviewCycleHeading">
            <header class="overview-section-head">
                ...
                <p>عدد الطلبات في كل لوحة — {{ $period_label ?? '' }}
                    @if (isset($cycle_total_active))
                        — {{ $cycle_total_active }} حالة نشطة في الفترة
                    @endif
                </p>
            </header>
            @if (! empty($cycle_cards))
    ...
    @if (! empty($case_strip))
        <section class="overview-section" aria-label="حالة الحالات">
            ...
            <div class="overview-cases-strip" id="overviewCasesStrip">
                @if (isset($case_strip['waiting_return']))
                    <button type="button" class="overview-case-link overview-case-link--wait" data-goto-cases="waiting_return">
                        ...
                        <span id="overviewWaitingCount" class="overview-case-link__count" data-server-rendered="1">{{ $case_strip['waiting_return'] }}</span>
                    </button>
                @endif
                @if (isset($case_strip['awaiting_cashier']))
                    <button type="button" class="overview-case-link overview-case-link--cashier" data-goto-cases="awaiting_cashier">
                        ...
                        <span id="overviewCashierCount" class="overview-case-link__count" data-server-rendered="1">{{ $case_strip['awaiting_cashier'] }}</span>
                    </button>
                @endif
                @if (isset($case_strip['awaiting_assignment']))
                    <button type="button" class="overview-case-link overview-case-link--assignment" data-goto-cases="awaiting_assignment">
                        ...
                        <span id="overviewAssignmentCount" class="overview-case-link__count" data-server-rendered="1">{{ $case_strip['awaiting_assignment'] }}</span>
                    </button>
                @endif
                @if (isset($case_strip['in_progress']))
                    <button type="button" class="overview-case-link overview-case-link--progress" data-goto-cases="in_progress">
                        ...
                        <span id="overviewProgressCount" class="overview-case-link__count" data-server-rendered="1">{{ $case_strip['in_progress'] }}</span>
                    </button>
                @endif
                @if (isset($case_strip['delivered']))
                    <button type="button" class="overview-case-link overview-case-link--done" data-goto-cases="delivered">
                        ...
                        <span id="overviewDeliveredCount" class="overview-case-link__count" data-server-rendered="1">{{ $case_strip['delivered'] }}</span>
                    </button>
                @endif
            </div>
        </section>
